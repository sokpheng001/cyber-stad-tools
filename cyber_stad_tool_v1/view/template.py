...
class Template(exploit_option.Exploit, reconnaissance_option.Reconnaissance, generate_report.Report):
    ...
    def start_options(self, opt):
        try:
            ...
            # report is here
            ...
            elif opt.lower().replace(" ", "") == "reload" or opt.lower().replace(" ",
                                                                                 "") == "restart" or opt.lower().replace(
                    " ", "") == "r":
                # reload our tools
                ...
                print(
                    Fore.RED + "=> Invalid command, it looks like you want to use \'clear\' command :) .!!!" + Style.RESET_ALL);
                self.choose_option()
            elif opt == "99":
                exit_message.exit_tool_message()
                sys.exit(0)
